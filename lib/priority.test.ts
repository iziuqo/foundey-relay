...
import { items, demoInjections, team } from "./seed";
...
describe("mayNeedHelp", () => {
  ...
  it("flags an item the moment helpAsked is set, whatever its age (§8.6)", () => {
    const freshlyOpened = items.find((i) => i.id === "it-01")!;
    expect(freshlyOpened.helpAsked).toBeFalsy();
    expect(mayNeedHelp(freshlyOpened, undefined, NOW)).toBe(false);
    expect(mayNeedHelp({ ...freshlyOpened, helpAsked: true }, undefined, NOW)).toBe(true);
  });
});
