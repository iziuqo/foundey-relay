...
export function scoreItem(item: Item, now: Date): ScoreResult {
  ...
  const tier: Tier = item.safety || score >= 60 ? 'now' : score >= 30 ? 'next' : 'later'
...
export function compareRanked(a: Ranked, b: Ranked): number {
  ...
  if (a.item.ordersBlocked !== b.item.ordersBlocked) return b.item.ordersBlocked - a.item.ordersBlocked
...
export function loadFor(doNowCount: number, upNextCount: number): LoadResult {
  ...
  const label: LoadResult['label'] = points >= 12 || doNowCount >= 4 ? 'full' : points >= 6 ? 'busy' : 'light'
  return { points, label }
}

/** §4.5. Item based, never person based. */
export function mayNeedHelp(item: Item, assignee: Person | undefined, now: Date): boolean {
  if ((item.notMineCount ?? 0) >= 2) return true
  if (item.status === 'in_progress' && assignee?.currentTaskId === item.id && assignee.currentTaskStartedAt) {
...
export function reassignedCause(originalCause: string, managerFirstName: string): string {
