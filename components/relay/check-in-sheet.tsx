    ...
    <div className="flex flex-col gap-4 p-4">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-(length:--text-meta) text-(--text-2)">{person.role}</p>
        </div>
      </div>

      {rows.length === 0 ? (
        <p className="text-(length:--text-meta) text-(--text-2)">{copy.team.checkInEmpty}</p>
      ) : (
          ...
          {rows.map(({ item, result }) => {
            ...
            return (
              <li
                ...
                className={cn(
                  "flex min-h-(--size-row-min) items-center gap-3 border-b border-(--border-1) px-2 last:border-0",
                  highlighted && "rounded-(--radius-control) bg-(--surface-2)",
                )}
              >
                <TierIcon tier={tier} safety={item.safety} className={cn("shrink-0", tierFgClass[tier])} />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-(length:--text-row) leading-(length:--leading-row) font-medium text-(--text-1)">
                    {item.title}
                  </p>
                  <p className="truncate text-(length:--text-meta) text-(--text-2)">{item.cause}</p>
                </div>
